...
## How far apart should deep buttons be spaced?

Deep buttons are spaced 100–120 mm apart, horizontally and vertically, in a diamond grid with alternate rows offset by half a spacing. Closer than 100 mm the panel looks busy; wider than 120 mm the pleats between the buttons sag.
...
*Figure: Top view of a deep-buttoned panel. Buttons sit at every diamond node and the pleats run between them; rows are offset by half a column-spacing to form the diamond pattern, symmetrical about the panel’s centre lines.*
...
The technique requires the [long straight buttoning needle](https://www.learntoupholster.com/the-toolkit) and a steady pull. Thread the twine through the button shank; thread the buttoning needle with both ends of the twine; push the needle down through the show fabric, the wadding, the calico, the stuffing, and out the back. Have a helper hold the backing patch ready; slip the twine under the patch and tie a [slip knot](https://www.learntoupholster.com/knots-and-stitches).

Now pull. The button compresses down into the pad, creating the dimple. Pull until the dimple is the depth you want (about 25 mm for shallow buttoning, 40 mm for deep). Tighten the slip knot, secure with a [square knot](https://www.learntoupholster.com/knots-and-stitches), trim the twine. Move to the next button.
...
*Figure: Cross-section showing how each button is pulled down through the pad: twine from the button shank, through every layer, tied off against a backing patch on the underside. The fabric pleats up between adjacent buttons.*
